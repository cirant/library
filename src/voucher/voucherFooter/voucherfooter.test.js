...
describe('VoucherFooter tests', () => {
  ...
  test('Should render component unorder', () => {
    const component = render(<Component items={{ ...items, typeList: 'unorder' }} />);
    ...
    expect(component.getAllByTestId('test-bullet-unorder').length).toEqual(items.bullets.length);
  });

  test('Should render component order', () => {
    const component = render(<Component items={{ ...items, typeList: 'order' }} />);
    items.bullets.forEach((bullet, index) => {
      expect(component.getByText(new RegExp(index + 1))).toBeTruthy();
      expect(component.getByText(bullet.text)).toBeTruthy();
    });
    expect(component.getAllByTestId('test-bullet-order').length).toEqual(items.bullets.length);
  });

  test('Should render component with breakColumn prop', () => {
    const component = render(<Component breakColumn="1" items={{ ...items, typeList: 'order' }} />);
